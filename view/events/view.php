		...
		<div class="event">	
			...
			<div class="span7">
				...
				<div class="event-when">
					...
					<span class="event-time"><?php echo str_replace(':','h',substr($event->time,0,5));?></span>
				...
				<div class="event-location">
					...
					<h2 class="event-city"><?php echo $event->city;?></h2>
					<h3 class="event-address"><?php echo stripcslashes($event->address);?></h3>
				</div>

				<div class="event-organizer">
					<i class="icon icon-home"></i>
					<a class="event-user" href=""><?php echo $event->getLogin();?></a>
					<span class="event-age">(<?php echo $event->getAge();?> ans )<span>
				</div>


				<div class="event-participants">
					<i class="icon icon-user"></i> <strong>Participants :</strong>
					...
					<?php if($this->session->user()->isLog()): ?>
						<?php if(!$event->isAdmin($this->session->user()->getID())): ?>
							<?php if(isset($event->UserParticipation)): ?>
								...
								<a class="btn btn-link" href="<?php echo Router::url('events/removeParticipant?event_id='.$event->id.'&user_id='.$this->session->user()->getID());?>"><i class="icon-remove icon-white"></i> Annuler</a>
							<?php else: ?>
